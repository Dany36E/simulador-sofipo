...
def main():
    ...
    if len(inversiones_seleccionadas) > 0:
        ...
        if analisis:
            ...
            # ====================================================================
            # SISTEMA DE GAMIFICACIÓN - LOGROS Y BADGES
            # ====================================================================
            ...
            # Calcular nivel y puntos máximos
            puntos_maximos = 115  # 25 + 30 + 20 + 15 + 25
            porcentaje_completado = (puntos / puntos_maximos) * 100
            ...
            for idx, logro in enumerate(logros):
                with cols_logros[idx % 3]:
                    if logro["desbloqueado"]:
                        ...
                        ">
                            <div style="font-size: 40px; text-align: center;">{logro["badge"]}</div>
                            <div style="font-weight: bold; text-align: center; margin: 10px 0;">{logro["titulo"]}</div>
                            <div style="font-size: 12px; text-align: center; opacity: 0.9;">{logro["descripcion"]}</div>
                            <div style="text-align: center; margin-top: 10px; font-weight: bold;">+{logro["puntos"]} pts</div>
                        </div>
                        """, unsafe_allow_html=True)
                    else:
                        st.markdown(f"""
                        ...
                        ">
                            <div style="font-size: 40px; text-align: center; filter: grayscale(100%);">{logro["badge"]}</div>
                            <div style="font-weight: bold; text-align: center; margin: 10px 0;">{logro["titulo"]}</div>
                            <div style="font-size: 12px; text-align: center;">{logro["descripcion"]}</div>
                            <div style="text-align: center; margin-top: 10px; opacity: 0.5;">+{logro["puntos"]} pts</div>
                        </div>
                        """, unsafe_allow_html=True)
